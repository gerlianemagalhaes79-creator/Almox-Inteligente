...
export default function App() {
  ...
  const [authRole, setAuthRole] = useState<'ADMIN' | 'LÍDER' | 'SETOR'>('SETOR');
        ...
        <body>
          ...
          <div class="batch-header">
            <div class="batch-header-left">
              ...
              `}
              <div>
                <div style="font-weight: 900; font-size: 12px; color: #0F172A; text-transform: uppercase;">Policlínica Bernardo Félix da Silva</div>
                <div style="font-size: 9px; font-weight: 700; color: #0284C7;">Consórcio CPSMS • Almoxarifado Central</div>
              </div>
            </div>
            <div style="text-align: right;">
              <div style="font-size: 11px; font-weight: 900; color: #0F172A;">LOTE DE FOLHAS DE SOLICITAÇÃO</div>
              <div style="font-size: 9px; color: #64748B;">Período: <strong>${periodStr}</strong> • ${filteredRequests.length} solicitações</div>
            </div>
          </div>
            ...
            return `
              <div class="request-card">
                <div class="card-header-bar">
                  <span class="protocol-tag">FOLHA DE SOLICITAÇÃO #${req.id.slice(-6).toUpperCase()}</span>
                ...
                <table class="header-table">
                  <tr>
                    <td style="width: 50%;"><strong>Data:</strong> ${new Date(req.date).toLocaleDateString('pt-BR')} às ${new Date(req.date).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</td>
                    <td style="width: 50%;"><strong>Setor Solicitante:</strong> <span style="font-weight: 900; color: #0369A1;">${req.sector}</span></td>
                  </tr>
                  <tr>
                    <td colspan="2"><strong>Solicitante:</strong> ${req.requesterEmail || 'Não informado'}</td>
                  </tr>
                ...
                <h3 style="margin: 6px 0 3px 0; font-size: 8.5px; border-bottom: 1px solid #0F172A; padding-bottom: 2px; text-transform: uppercase; font-weight: 800; color: #334155;">
                  ITENS DA SOLICITAÇÃO (Para separação física)
                </h3>
                <table class="items-table">
                  <thead>
                    <tr>
                      <th style="width: 32%;">Produto / Descrição</th>
                      <th style="width: 10%; text-align: center;">Qtd Solic.</th>
                      <th class="blank-col" style="width: 14%; text-align: center;">Qtd Separada</th>
                      <th style="width: 44%;">Lotes em Estoque (Sugestão FEFO)</th>
                    </tr>
                  </thead>
                  <tbody>
                    ${reqItemsList.map(item => {
                      ...
                      if (hasMultipleBatches) {
                        batchDisplay = `
                          <div class="lot-warning-box">⚠️ ${productBatches.length} LOTES EM ESTOQUE</div>
                          <div class="lot-item-line">
                            ${productBatches.map((b, bIdx) => {
                              ...
                              return `<div>• Lote ${b.batch_number || 'S/N'}: ${b.quantity} un (Val: ${expDate})${isFirst}</div>`;
                            }).join('')}
                          </div>
                        `;
                        ...
                        batchDisplay = `
                          <div style="font-size: 8px; color: #1C1917;">
                            <strong>Lote:</strong> ${b.batch_number || 'S/N'} • <strong>Saldo:</strong> ${b.quantity} un (Val: ${expDate})
                          </div>
                        `;
                      } else {
                        batchDisplay = `
                      ...
                      return `
                        <tr>
                          <td style="font-weight: bold; font-size: 8.5px;">
                            ...
                            ${hasMultipleBatches ? `<div class="badge-multiple">⚠️ MÚLTIPLOS LOTES (${productBatches.length})</div>` : ''}
                          </td>
                ...
                <div class="footer">Gerado em ${new Date().toLocaleString('pt-BR')} • Policlínica Bernardo Félix da Silva</div>
              </div>
            `;
          }).join('')}
          <script>
  ...
  const handleDeleteRequest = async (requestId: string) => {
    ...
    // Se a solicitação foi aprovada, entregue ou está em processo de atendimento, não é mais permitido apagar
    ...
    if (isApprovedOrDelivered) {
      showToast("Operação não permitida: solicitações aprovadas ou já entregues não podem ser apagadas.", "error");
      return;
    }
  ...
  const handlePrintSingleRequest = async (request: MaterialRequest) => {
    ...
    const content = `
      ...
      <html lang="pt-BR">
        <head>
          <meta charset="UTF-8">
          <title>Folha de Solicitação de Material - ${protocolCode}</title>
          <style>
            @page {
              size: A4 portrait;
          ...
          <div class="sheet-container">
            ...
            <div class="header-banner">
              ...
              <div class="header-right">
                <div class="doc-title">Folha de Solicitação de Material</div>
                <div class="protocol-badge">${protocolCode}</div>
                <div class="status-badge ${request.status === 'ENTREGUE' ? 'status-delivered' : ''}">
                  Status: ${request.status === 'PENDENTE' ? 'Pendente de Triagem' : request.status === 'EM_SEPARACAO' ? 'Em Separação Física' : request.status}
                </div>
              </div>
            </div>
            ...
            <div class="meta-card">
              ...
              <div class="meta-item">
                <span class="meta-label">Data / Hora da Solicitação</span>
                <span class="meta-val">${dateStr} às ${timeStr}</span>
              </div>
              <div class="meta-item">
              ...
              ${request.observation ? `
                <div class="meta-item meta-full">
                  <span class="meta-label">Justificativa / Observações do Solicitante</span>
            ...
            <div class="section-header">
              <span>Itens Requisitados (${reqItemsList.length} itens • ${totalQtyRequested} unidades no total)</span>
              <span style="font-size: 8px; color: #64748B;">Guia para Separação Física e Conferência</span>
            </div>
              ...
              <tbody>
                ${reqItemsList.map((item, idx) => {
                  ...
                  if (!canViewStockQuantity) {
                      ...
                      <div>
                        ...
                        <div class="lot-suggestion">
                          ${productBatches.map((b, bIdx) => {
                            ...
                            const fefo = bIdx === 0 ? '<span class="fefo-badge">PRIORITÁRIO</span> ' : '';
                            return `<div>• ${fefo}Lote: <strong>${b.batch_number || 'S/N'}</strong> (${b.quantity} un - Val: ${exp})</div>`;
                          }).join('')}
                        </div>
                      </div>
                    `;
                    ...
                    stockInfo = `
                      <div class="lot-suggestion">
                        <strong>Lote:</strong> ${b.batch_number || 'S/N'} • <strong>Saldo:</strong> ${b.quantity} un • <strong>Val:</strong> ${exp}
                      </div>
                    `;
                  } else {
            ...
            <div class="guidelines-box">
              <strong>⚠️ PROTOCOLO DE CONFERÊNCIA:</strong>
              1. A separação física deve obrigatoriamente priorizar os lotes com vencimento mais próximo (FEFO).
  ...
  const handleApproveAndDeliverNewRequest = async (requestId: string, currentRequestItems: RequestItem[]) => {
    try {
      ...
      for (const reqItem of currentRequestItems) {
        ...
        // Se o usuário selecionou um lote específico, priorizar esse lote na baixa
  ...
  const handleModalRequestDevolution = async (params: {
    ...
    try {
      ...
      showToast("Solicitação de devolução enviada com sucesso!", "success");
      setShowDevolutionModal({ show: false });
    } catch (error: any) {
      console.error("Erro ao solicitar devolução:", error);
      showToast(`Erro ao solicitar devolução: ${error.message}`, "error");
    ...
    <div className="min-h-screen bg-[#F5F5F4] text-[#1C1917] font-sans">
                              ...
                              <div className="flex flex-col">
                                <div className="flex items-center gap-2 group/name flex-wrap">
                                  ...
                                  <button
                                    ...
                                    title={`Ver todas as saídas de ${group.name} (setor, quantidades, lotes e datas)`}
                                  >
                                    <Eye size={15} />
                                    <span className="text-[10px] font-extrabold">Saídas</span>
                                  </button>
                        ...
                        <td className="px-6 py-4.5 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <button
                              ...
                              title={`Consultar histórico de saídas de ${group.name}`}
                            >
                              <Eye size={14} />
                        ...
                        <tr key={item.id} className="bg-slate-50/70 hover:bg-blue-50/50 transition-all border-l-4 border-blue-600">
                          ...
                          <td className="px-6 py-3.5 text-right space-x-1.5">
                            <button 
                              ...
                              title={`Ver saídas do lote ${item.batch_number || 'Sem lote'}`}
                            >
                              <Eye size={15} />
                            </button>
                            <button 
                              onClick={(e) => { 
                                ...
                                setBulkEntry({
                                  ...
                                  room: item.room || (inventoryLocation === 'Farmácia' ? 'Farmácia' : 'Almoxarifado Principal'),
                                  items: [{
                                    id: Math.random().toString(36).substr(2, 9),
                                    name: item.name,
                                    initial_quantity: 1,
                  ...
                  <div className="flex items-start gap-4 pt-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-slate-500 text-xs font-medium mt-1 leading-snug">
                        Calcula a quantidade necessária de cada item para durar até o mês de interesse.
                      </p>
                    </div>
                  </div>
                ...
                <div className="bg-white p-6 rounded-3xl border border-blue-100/90 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden group flex flex-col justify-between h-full">
                  ...
                  <div className="flex items-start gap-4 pt-2">
                    ...
                    <div>
                      ...
                      <h3 className="text-base font-black text-slate-900 leading-tight">Balanço de Estoque & Lotes</h3>
                      <p className="text-slate-500 text-xs font-medium mt-1 leading-snug">
                        Conferência física por tipo de material, auditoria de lotes, datas de validade e apuração de sobras e faltas
                      </p>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 lg:p-8 shadow-2xl border border-slate-200 space-y-6 overflow-hidden my-auto">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-slate-900">Nova Saída de Materiais</h3>
            ...
            <form onSubmit={handleTransaction} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-600 mb-1.5">Motivo da Saída *</label>
                  <select 
                    className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold focus:ring-2 focus:ring-blue-500/20"
                    value={exitReason}
              ...
              {(exitReason === 'vencido' || exitReason === 'perda') && (
                <div>
                  <label className="block text-[11px] font-bold text-slate-600 mb-1.5">Observação / Justificativa *</label>
                  <input 
                    type="text"
                    required
          ...
          <div className="bg-white rounded-3xl max-w-4xl w-full p-6 lg:p-8 shadow-2xl border border-slate-200 space-y-6 max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-slate-900">Itens em Nível Crítico / Alerta</h3>
                  <p className="text-xs text-slate-500">Materiais com estoque zerado, baixo ou próximos ao vencimento</p>
                </div>
              </div>
              <button 
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            ...
            <p className="text-xs text-slate-600">
              Tem certeza que deseja excluir esta movimentação? Informe o motivo abaixo para fins de auditoria:
            </p>
            <textarea 
              rows={3}
              required
          ...
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 lg:p-8 shadow-2xl border border-slate-200 space-y-6">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-slate-900">Configurações do Sistema</h3>
            ...
            {settingsTab === 'logo' && (
              <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-center justify-between">
                  <div>
                    <h5 className="font-bold text-sm text-slate-800">Logo Principal (Ícone)</h5>
                    <p className="text-xs text-slate-500">Exibida na barra lateral e cabeçalho</p>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Exibida no centro da página de autenticação</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <label className="px-3 py-1.5 bg-blue-700 text-white rounded-xl text-xs font-bold cursor-pointer hover:bg-blue-800 transition-all">
                      Upload
                ...
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-center justify-between">
                  <div>
                    <h5 className="font-bold text-sm text-slate-800">Papel Timbrado (Relatórios PDF)</h5>
                    <p className="text-xs text-slate-500">Cabeçalho oficial impresso nos documentos</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <label className="px-3 py-1.5 bg-blue-700 text-white rounded-xl text-xs font-bold cursor-pointer hover:bg-blue-800 transition-all">
                      Upload
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Recalcula consumo semanal e previsão de término</p>
                  </div>
                  <button 
                    onClick={() => showToast("Auditoria e recálculo de estoque realizados.", "success")}
            ...
            {settingsTab === 'info' && (
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-600 mb-1">Endereço da Unidade</label>
                  <input 
                    type="text"
                    className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold"
      ...
      {showMergeSuppliers && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Fornecedor de Origem (Será substituído)</label>
              <input 
                type="text"
      ...
      {showMergeItems && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Material Origem (Será renomeado)</label>
              <input 
                type="text"
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold"
      ...
      {showRoomInventoryModal && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              <h3 className="text-base font-black text-slate-900">Inventário Físico por Sala</h3>
            ...
            <div>
              ...
              <input 
                type="text"
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold"
                placeholder="Ex: Sala A / Consultório 1"
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button 
                ...
                className="px-5 py-2 bg-blue-700 text-white text-xs font-bold rounded-xl hover:bg-blue-800 transition-all disabled:opacity-50"
              >
                {isUpdatingCategory ? 'Atualizando...' : 'Salvar Alteração'}
              </button>
            </div>
          </div>
        </div>
      ...
      {showCriticalReportModal && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              <h3 className="text-base font-black text-slate-900">Exportar Relatório de Alertas</h3>
            ...
            <div>
              ...
              >
                ...
                <option value="expiry">Itens Vencidos / Próximos ao Vencimento</option>
              </select>
            </div>
            <div className="flex items-center justify-end gap-3 pt-2">
              <button 
      ...
      {showPurchasePlanningModal && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Mês Alvo</label>
                <select 
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold"
                  value={planningTargetMonth}
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Margem de Segurança</label>
                <select 
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold"
                  value={planningSafetyOption}
                  onChange={e => setPlanningSafetyOption(e.target.value as any)}
                >
                  <option value="standard_8w">Padrão (8 Semanas de Reserva)</option>
                  <option value="margin_10">Margem Adicional de 10%</option>
                  <option value="margin_20">Margem Adicional de 20%</option>
                  <option value="none">Sem Margem (Consumo Líquido)</option>
                </select>
              </div>
              <div>
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              <label htmlFor="onlyDeficit" className="text-xs font-bold text-slate-700 cursor-pointer">
                Exibir apenas itens com necessidade de compra (Déficit &gt; 0)
              </label>
            </div>
